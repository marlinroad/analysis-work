...
def make_portfolio_values(closes):
    return pd.DataFrame({
        "date": pd.date_range("2024-10-01", periods=len(closes), freq="D"),
        "close": closes,
    })


def make_config(rows):
    return pd.DataFrame(rows, columns=["symbol", "date", "shares", "total_shares", "cashflow"])
...
def test_original_columns_are_preserved_before_new_columns():
    ...
    assert list(result.columns) == ["date", "close", "source", "cashflow", "dlyret"]
