...
def df_config():
    return pd.DataFrame([
        ...
        ("AAA", "2023-1-4", 20, 30),
        ("BBB", "2023-1-2", 5, 5),
    ], columns=["symbol", "date", "shares", "total_shares"])
...
def test_output_starts_at_first_transaction_date(df_stocks, df_config):
    ...
    assert df_portfolio_values["date"].min() == pd.Timestamp("2023-01-01")


def test_value_before_topup(df_stocks, df_config):
    df_portfolio_values = calculate_portfolio_daily_values(df_stocks, df_config)
    row = df_portfolio_values.loc[df_portfolio_values["date"] == pd.Timestamp("2023-01-03")]
...
def test_value_after_topup_with_forward_filled_price(df_stocks, df_config):
    df_portfolio_values = calculate_portfolio_daily_values(df_stocks, df_config)
    row = df_portfolio_values.loc[df_portfolio_values["date"] == pd.Timestamp("2023-01-04")]
...
def test_value_on_last_day(df_stocks, df_config):
    df_portfolio_values = calculate_portfolio_daily_values(df_stocks, df_config)
    row = df_portfolio_values.loc[df_portfolio_values["date"] == pd.Timestamp("2023-01-06")]
